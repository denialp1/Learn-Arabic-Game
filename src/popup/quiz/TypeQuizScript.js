...
function setExercise(count) {
  if (count >= 1 && count <= exercises.length) {
    ...
    // answerEng = exercise.english.trim().split(/\s+/); 
    // answerAra = exercise.arabic.trim().split(/\s+/);
    setItems();
    setInputArea();
  }
}
...
function setInputArea() {
  ...
  const textarea = document.getElementById('user-input'); // textarea to autformat nums
